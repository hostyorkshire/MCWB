...
if [ -n "$DNS_SERVERS" ]; then
    ...
    if host github.com >/dev/null 2>&1; then
        ...
        echo ""
        echo "   Quick fix options (choose one):"
        echo ""
        echo "   Option 1 - For systemd-resolved (recommended for modern systems):"
        echo "   sudo nano /etc/systemd/resolved.conf"
        echo "   # Uncomment and set: DNS=8.8.8.8 1.1.1.1"
        echo "   sudo systemctl restart systemd-resolved"
        echo ""
        echo "   Option 2 - For NetworkManager:"
        echo "   sudo nmcli connection modify \"Your-WiFi-Name\" ipv4.dns \"8.8.8.8 1.1.1.1\""
        echo "   sudo nmcli connection up \"Your-WiFi-Name\""
        echo ""
        echo "   Option 3 - Quick temporary fix (legacy or emergency):"
